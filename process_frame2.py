...
class ProcessFrame:
    def __init__(self, thresholds, flip_frame=False):
        ...
        self.FEEDBACK_ID_MAP = {
            0: ('Penchez-vous plus en arrière', 215, (0, 153, 255)), #[0][...]
            1: ('Penchez-vous plus en avant', 215, (0, 153, 255)),#[1][...]
            2: ('Reculez vos genoux', 170, (255, 80, 80)),#[2][...]
            3: ('Descendez moins bas', 125, (255, 80, 80))#[3][...]
        }


    def _get_state(self, knee_angle):  # pour le nbr de rep
        # normal->trans->pass
        knee = None  # ?

        if self.thresholds['HIP_KNEE_VERT']['NORMAL'][0] <= knee_angle <= self.thresholds['HIP_KNEE_VERT']['NORMAL'][
            1]:  # si knee_angle est compris entre ['NORMAL'][0] et ['NORMAL'][1] alors on est à l'etat normal
            knee = 1
        elif self.thresholds['HIP_KNEE_VERT']['TRANS'][0] <= knee_angle <= self.thresholds['HIP_KNEE_VERT']['TRANS'][
            1]:  # si knee_angle est compris entre ['TRANS'][0] et ['TRANS'][1] alors on est à l'etat TRANS
            knee = 2
        elif self.thresholds['HIP_KNEE_VERT']['PASS'][0] <= knee_angle <= self.thresholds['HIP_KNEE_VERT']['PASS'][
            1]:  # si knee_angle est compris entre ['PASS'][0] et ['PASS'][1] alors on est à l'etat PASS
            knee = 3

        return f's{knee}' if knee else None  # return s1, s2 ou s3 si knee sinon none avec f pour le formatage (compacte) en string

    def _update_state_sequence(self, state):  # pour compter les reps en suivant l'ecolution des etats

        if state == 's2':  # s1=normal, s2=trans, s3=pass
    ...
    def _show_feedback(self, frame, c_frame, dict_maps, lower_hips_disp):  # affichage -> dessin
    ...
    def _draw_line_4_correction(self, frame, c_frame, dict_maps, shldr_coord, hip_coord, knee_coord):

        for idx in np.where(c_frame)[0]:
            if dict_maps[idx][0] == 'Penchez-vous plus en arrière' or dict_maps[idx][0] == 'Penchez-vous plus en avant':
                cv2.line(frame, hip_coord, shldr_coord, self.COLORS['red'],4, lineType=self.linetype)
                cv2.line(frame, hip_coord, shldr_coord, self.COLORS['red'],4, lineType=self.linetype)

            elif dict_maps[idx][0] == 'Reculez vos genoux' or dict_maps[idx][0] == 'Descendez moins bas':
                cv2.line(frame, hip_coord, knee_coord, self.COLORS['red'],4, lineType=self.linetype)
                cv2.line(frame, hip_coord, knee_coord, self.COLORS['red'],4, lineType=self.linetype)

        return frame

    def process(self, frame: np.array, pose):  # fonction principale
        play_sound = None
        ...
        if keypoints.pose_landmarks:
            ...
            # Camera is aligned properly.
                ...
                if dist_l_sh_hip > dist_r_sh_hip:  # connaitre la direction dans laquelle regarde de la personne ?
                ...
                else:  # connaitre la direction dans laquelle regarde de la personne ?
                ...
                # Plot landmark points in yellow
                ...
                if current_state == 's1':# debout

                    if len(self.state_tracker['state_seq']) == 3 and not self.state_tracker['INCORRECT_POSTURE']:#a realise un cycle/rep
                        self.state_tracker['SQUAT_COUNT'] += 1
                        play_sound = str(self.state_tracker['SQUAT_COUNT'])

                    elif 's2' in self.state_tracker['state_seq'] and len(self.state_tracker['state_seq']) == 1:#a realise un demi cycle/rep
                    ...
                    self.state_tracker['state_seq'] = [] # on reinitialise les conteurs
                ...
                frame = self._draw_line_4_correction(frame,  self.state_tracker['COUNT_FRAMES'], self.FEEDBACK_ID_MAP, shldr_coord, hip_coord,
                                                     knee_coord)
